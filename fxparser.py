...
class Parser:
        ...
        if not res.error and self.current_tok.type != TT_EOF:
            return res.failure(
                InvalidSyntaxError(
                    ...
                    "Expected '+', '-', '*', '/', '^', '==', '!=', '<', '>', '<=', '>=', 'and', 'or', 'let', 'if', 'for', 'while', 'fex', int, float, identifier, '+', '-', '(', '[', 'not', 'return', 'continue', 'break', 'import', 'from' or 'end'",
                )
            )
        return res
    ...
    def statements(self) -> ParseResult:
        ...
        return res.success(ListNode(statements, pos_start, statements[-1].pos_end if statements else pos_start))
    ...
    def dict_expr(self) -> ParseResult:  # ParseResult is used to return error or node
        res = ParseResult()
        ...
        return res.success(DictNode(key_value_pairs, self.current_tok.pos_start, self.current_tok.pos_end))
