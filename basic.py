...
class Error:
    def __init__(self, pos_start: None, pos_end: None, error_name:str, details:str):
...
class FuncDefNode:
    def __init__(
        self, var_name_tok, arg_name_toks, body_node, should_auto_return=False
    ):
        self.var_name_tok = var_name_tok
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected 'let', 'if', 'for', 'while', 'fex', int, float, identifier, '+', '-', '(', '[' or 'not'",
                )
            )
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected int, float, identifier, '+', '-', '(', '[', 'if', 'for', 'while', 'fex' or 'not'",
                )
            )
    ...
    def atom(self):
        ...
        return res.failure(
            InvalidSyntaxError(
                tok.pos_start,
                tok.pos_end,
                "Expected int, float, identifier, '+', '-', '(', '[', IF', 'for', 'while', 'fex'",
            )
        )
...
class BuiltInFunction(BaseFunction):
        ...
        if isinstance(list_, Number):
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    "Append method cannot be called on a number",
                    exec_ctx,
                )
            )
    ...
    def execute_pop(self, exec_ctx):
        ...
        if isinstance(list_, Number):
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    "Pop method cannot be called on a number",
                    exec_ctx,
                )
            )
        ...
        try:
            element = (
                ...
                if isinstance(list_, List)
                else list_.value.pop(index.value)
            )
        except:
            return RTResult().failure(
                RTError(
        ...
        if isinstance(list_, Number):
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    "Len method cannot be called on a number",
                    exec_ctx,
                )
            )

        return RTResult().success(
            Number(len(list_.elements) if isinstance(list_, List) else len(list_.value))
        )

    execute_len.arg_names = ["list"]

    def execute_eval(self, exec_ctx):
        eval_str = exec_ctx.symbol_table.get("string")
        try:
            result = eval(eval_str)
        except Exception as e:
            return RTResult().failure(
                RTError(self.pos_start, self.pos_end, f"{str(e)}", exec_ctx)
            )
    ...
    def execute_type(self, exec_ctx):
        value = exec_ctx.symbol_table.get("value")
        return RTResult().success(String(value.__class__.__name__))
    ...
    def execute_root(self, exec_ctx):
        value = exec_ctx.symbol_table.get("value")
        power = (
            exec_ctx.symbol_table.get("power")
            if exec_ctx.symbol_table.get("power")
            else Number(2)
        )

        if not isinstance(value, Number) or not isinstance(power, Number):
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
        ...
        return RTResult().success(Number(value.value ** (1 / power.value)))

    execute_root.arg_names = ["value", "power"]
    ...
    def execute_round(self, exec_ctx):
        ...
        if not isinstance(value, Number):
            return RTResult().failure(
                RTError(
                    self.pos_start, self.pos_end, "Argument must be a number", exec_ctx
                )
            )
    ...
    def execute_factorial(self, exec_ctx):
        ...
        return RTResult().success(Number(math.factorial(value.value)))
    ...
    def execute_sum(self, exec_ctx):
        ...
        elements = [x.value for x in list_.elements]
        try:
            return RTResult().success(Number(sum(elements)))
        except:
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    "Cannot sum elements of list (are they all numbers?)",
                    exec_ctx,
                )
            )
    ...
    def execute_min(self, exec_ctx):
        ...
        elements = [x.value for x in list_.elements]
        try:
            return RTResult().success(Number(min(elements)))
        except:
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    "Cannot find minimum of list (are they all numbers?)",
                    exec_ctx,
                )
            )
    ...
    def execute_max(self, exec_ctx):
        ...
        elements = [x.value for x in list_.elements]
        try:
            return RTResult().success(Number(max(elements)))
        except:
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    "Cannot find maximum of list (are they all numbers?)",
                    exec_ctx,
                )
            )
    ...
    def execute_range(self, exec_ctx):
        ...
        step = (
            exec_ctx.symbol_table.get("step")
            if exec_ctx.symbol_table.get("step")
            else Number(1)
        )

        if not all(isinstance(x, Number) for x in (start, end, step)):
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
        ...
        try:
            return RTResult().success(
                List([Number(x) for x in range(start.value, end.value, step.value)])
            )
        except:
            return RTResult().failure(
                RTError(self.pos_start, self.pos_end, "Invalid arguments", exec_ctx)
            )

    execute_range.arg_names = ["start", "end", "step"]

    def execute_convert(self, exec_ctx):
        value = exec_ctx.symbol_table.get("value")
        to_type = exec_ctx.symbol_table.get("to_type")

        if to_type.value.lower() == "number":
        ...
        elif to_type.value.lower() == "string":
            return RTResult().success(String(str(value)))
        elif to_type.value.lower() == "list":
            return RTResult().success(List([value]))
        else:
            return RTResult().failure(
                RTError(self.pos_start, self.pos_end, "Invalid type", exec_ctx)
            )

    execute_convert.arg_names = ["value", "to_type"]
    ...
    def execute_split(self, exec_ctx):
        value = exec_ctx.symbol_table.get("value")
        separator = (
            exec_ctx.symbol_table.get("separator")
            if exec_ctx.symbol_table.get("separator")
            else String(" ")
        )

        if not all(isinstance(x, String) for x in (value, separator)):
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
        ...
        return RTResult().success(
            List([String(x) for x in value.value.split(separator.value)])
        )
    ...
    def execute_join(self, exec_ctx):
        ...
        return RTResult().success(
            String(separator.value.join([x.value for x in list_.elements]))
        )
    ...
    def execute_strip(self, exec_ctx):
        ...
        return RTResult().success(String(value.value.strip()))

    execute_strip.arg_names = ["value"]
    ...
    def execute_replace(self, exec_ctx):
        ...
        if not all(isinstance(x, String) for x in (value, old, new)):
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
        ...
        return RTResult().success(String(value.value.replace(old.value, new.value)))

    execute_replace.arg_names = ["value", "old", "new"]
        ...
        try:
            ...
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    f"Error reading file '{filename.value}'",
                    exec_ctx,
                )
            )
        ...
        try:
            ...
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    f"Error writing to file '{filename.value}'",
                    exec_ctx,
                )
            )
    ...
    def execute_raise(self, exec_ctx):
        type_ = exec_ctx.symbol_table.get("type")
        error = exec_ctx.symbol_table.get("error")
        return RTResult().failure(
            Error(self.pos_start, self.pos_end, type_.value.capitalize(), error.value)
        )

    execute_raise.arg_names = ["type", "error"]
        ...
        try:
            ...
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    f"Error running command '{command.value}'",
                    exec_ctx,
                )
            )

    execute_system.arg_names = ["command"]

    def execute_random(self, exec_ctx):
        choices = exec_ctx.symbol_table.get("choices")
        count = (
            exec_ctx.symbol_table.get("count")
            if exec_ctx.symbol_table.get("count")
            else Number(1)
        )

        if isinstance(choices, Number):
            return RTResult().failure(
                RTError(
                    self.pos_start, self.pos_end, "Choices cant be a number", exec_ctx
                )
            )
        ...
        if not isinstance(count, Number):
            return RTResult().failure(
                RTError(
                    self.pos_start, self.pos_end, "Count must be a number", exec_ctx
                )
            )

        try:
            elements = (
                random.choices(choices.elements, k=count.value)
                if isinstance(choices, List)
                else random.choices(choices.value, k=count.value)
            )
        except:
            return RTResult().failure(
                RTError(self.pos_start, self.pos_end, "Invalid arguments", exec_ctx)
    ...
    execute_random.arg_names = ["choices", "count"]
    ...
    def execute_execute(self, exec_ctx):
        ...
        if not file.value.endswith(".fedxd"):
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    "Invalid file type, must be a '.fedxd' file",
                    exec_ctx,
                )
            )
        try:
            ...
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    f"Error reading file '{file.value}'\b{e}",
                    exec_ctx,
                )
            )
    ...
    execute_execute.arg_names = ["file"]
...
BuiltInFunction.raise_ = BuiltInFunction("raise")
...
BuiltInFunction.execute_file = BuiltInFunction("execute")
BuiltInFunction.import_file = BuiltInFunction("import")
...
global_symbol_table.set("FileRoot", String(os.path.dirname(os.path.abspath(__file__))))
...
global_symbol_table.set("raise", BuiltInFunction.raise_)
